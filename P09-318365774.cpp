...
int main()
{
	...
	movHelicZ = -50.0f;
	rotHelicop = 90.0f;
	float rotOffHeli = 90.0f;
	float rotOffDado = 15.0f;
	avanza = true;
	int aux = 0;
	float dist = 0.0f;
	float tablero = 100.0f;
	glfwSetTime(0);
	...
	while (!mainWindow.getShouldClose())
	{
		...
		if (!mainWindow.getbandera()) //Presionar H para mover el dado y J para reiniciar la tirada
		{
			if (movDadoY > 0.0 && movDadoX > -5.0)
			{
				movDadoX += movOffset * deltaTime;
				movDadoY -= movOffset * deltaTime;
				rotDado += rotOffDado * deltaTime;
			}
		}
		else 
		{
		...
		rotHelice += rotOffset * deltaTime;  
		
		dist += movOffset * deltaTime;
		if (aux == 0)
		{
			movHelicX += movOffset * deltaTime;
			if (dist >= tablero)
			{
				rotHelicop -= rotOffHeli * deltaTime;
				aux = 1;
				dist = 0.0;
			}
			else
		...
		//Helicóptero
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(movHelicX, 5.0f, movHelicZ));
		model = glm::scale(model, glm::vec3(0.2f, 0.2f, 0.2f));
		modelaux = model;
		model = glm::rotate(model, rotHelicop * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
